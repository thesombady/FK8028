...
def Initalize(numberOfAtoms1D: int, state: State) -> List[Atom]:
    ...
    average_velocity: Vector = np.array([0.0, 0.0, 0.0])
    ...
    for i in range(len(atoms)):
        atoms[i].velocity -= average_velocity / len(atoms)
...
def runSimulation(state: State, numberOfAtoms: int = 3):
    ...
    # allPosition: np.ndarray = np.zeros((numberOfAtoms ** 3, int(T/dt)), dtype = Atom)
    # allVelocity: np.ndarray = np.zeros((numberOfAtoms ** 3, int(T/dt), dtype = Atom)
    # allPotential: np.ndarray = np.zeros((numberOfAtoms ** 3, int(T/dt), dtype = Atom)
    ...
    while t < T:
        ...
        """
        if counter < 1000 and counter % 50 == 0:
...
if __name__ == '__main__':
    ...
    pos1 = np.array([0.07, 0.0, 0.0])
    pos2 = np.array([6.07, 0.0, 0.0])
    potential, force = LennardJones(pos1, pos2)

    new_pos1 = VerletMethodPosition(Atom('Ar:0', pos1, np.array([0.0, 0.0, 0.0])), 1e-14, force/ATOM_MASS)
    new_pos1 = validateBoundary(new_pos1)
    print(new_pos1)

    new_pos2 = VerletMethodPosition(Atom('Ar:1', pos2, np.array([0.0, 0.0, 0.0])), 1e-14, -force/ATOM_MASS)

    new_pos2 = validateBoundary(new_pos2)

    print(new_pos2)
    print(ATOM_MASS)
    print(SPEED_OF_LIGHT)
    print(GRAM_TO_EV)
    print(GEV_CONVERSION)
    """
