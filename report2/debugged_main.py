...
#type Vector = npt.ArrayLike[npt.float64] # np.ndarray type used when type-hinting
...
def LennardJones(pos1: np.ndarray, pos2: np.ndarray) -> Tuple[float, np.ndarray]:
    """
...
def VerletMethodPosition(atom: Atom, dt: float, acc: np.ndarray) -> np.ndarray:
    """
    ...
    """
    pos: np.ndarray = atom.position + atom.velocity * dt + acc / (2) * dt**2

    return pos


def VerletMethodVelocity(atom: Atom, dt: float, accNew: np.ndarray, accOld: np.ndarray) -> np.ndarray:
    """
        Computes the new velocity of an atom using the Verlet method.
        Force is in unit eV/Å, dt is in unit s and mass is in unit eV/c^2
    """
    vel: np.ndarray = atom.velocity + (accNew  + accOld) / (2) * dt
    return vel
...
def validateBoundary(pos: np.ndarray) -> np.ndarray:
    """
...
def kineticTemperature(velocities: List[np.ndarray]) -> Tuple[float, float]:
    """
...
def runSimulation(state: State, numberOfAtoms: int = 3):
    ...
    for i in range(len(atoms)):
        ...
        force: np.ndarray = np.array([0.0, 0.0, 0.0])
    ...
    while t < T:
        ...
        for i in range(len(atoms)):
            name: str = 'Ar:{}'.format(i)
            pos: np.ndarray = VerletMethodPosition(atoms[i], dt, Accelerations[i])
        ...
        for i in range(len(atoms)):
            ...
            potential: float = 0
            newForce: np.ndarray = np.array([0.0, 0.0, 0.0])
            for j in range(len(atoms)):
                if i == j:
                    continue
        ...
        velo: List[np.ndarray] = [allAtoms[name]['velocity'][-1] for name in allAtoms] # List of all current velocites
        ...
        if t < 2.5e-12 and counter % 50 == 0:

            for i in range(len(atoms)):
                velo[i] *= np.sqrt(94.4 / Temp)  # Use a variable here Target_temp = 94.4
